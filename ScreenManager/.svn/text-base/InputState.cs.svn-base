...
namespace WizardWars
{
    ...
    public class InputState
    {
        ...
        public bool IsNewLeftAnalog()
        {
            for (int i = 0; i < MaxInputs; i++)
            {
                if (IsNewLeftAnalog((PlayerIndex)i))
                    return true;
            }

            return false;
        }
        public bool IsNewLeftAnalog(PlayerIndex playerIndex)
        {
            return CurrentGamePadStates[(int)playerIndex].ThumbSticks.Left.X < -.7 &&
                LastGamePadStates[(int)playerIndex].ThumbSticks.Left.X > -.7;
        }
        public bool IsNewRightAnalog()
        {
            for (int i = 0; i < MaxInputs; i++)
            {
                if (IsNewRightAnalog((PlayerIndex)i))
                    return true;
            }

            return false;
        }
        public bool IsNewRightAnalog(PlayerIndex playerIndex)
        {
            return CurrentGamePadStates[(int)playerIndex].ThumbSticks.Left.X > .7 &&
                LastGamePadStates[(int)playerIndex].ThumbSticks.Left.X < .7;
        }
